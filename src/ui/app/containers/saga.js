...
export function* fetchLuckyNumber(username) {
  // TODO: What port is the service layer running on again?
  const port = '1337';
  const requestUrl = `http://localhost:${port}/lucky-number?username=${username}`;
...
export default function* sagaFunction() {
  yield takeLatest(DISPATCH_ACTIONS.GET_LUCKY_NUMBER, fetchLuckyNumber);
}
